...
export class TwitchIntegration {
  ...
  async getConfig(skipConnectionCheck = true): Promise<TwitchConfig | null> {
    try {
      ...
      return config as TwitchConfig | null;
    } catch (error) {
  ...
  async saveConfig(config: TwitchConfig): Promise<boolean> {
    try {
      ...
      if (!config.advanced) {
        config.advanced = defaultTwitchAdvancedConfig;
      }

      // Save to KV
      await kvService.set("integration:twitch", config);

      // Update agent config with Twitch settings
